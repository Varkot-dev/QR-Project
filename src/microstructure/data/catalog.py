...
from microstructure.data.binance import DumpFile, day_files, download, month_files
...
def _sync_one(
    ...
    """Download, ingest, verify, and rename one DumpFile.
    ...
    """
    dest = parquet_path(root, f.symbol, f.data_type, f.period)
    if not dest.exists():
...
def sync(
    ...
    for f in month_files(symbol, data_type, start, end):
        out.append(_sync_one(f, root, ingest, client=client))
    return out
...
def sync_days(
    ...
    """Ensure Parquet exists for every day in [start, end]; return the paths."""
    if data_type not in _INGESTERS:
        raise ValueError(
            f"unknown data_type {data_type!r}; expected one of {sorted(_INGESTERS)}"
        )
    ingest = _INGESTERS[data_type]
    out: list[Path] = []
    for f in day_files(symbol, data_type, start, end):
        out.append(_sync_one(f, root, ingest, client=client))
    return out
